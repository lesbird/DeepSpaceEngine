...
> Those layers now render as **lit procedural low-poly geometry** — lump-displaced **boulders** that sit
> half-embedded in the ground, and **trunk-and-canopy trees** (broadleaf blobs + tiered conifers) — with
> height-ramped colours, base-contact ambient occlusion and a biome tint so they belong in the palette,
> each instance **uniquely scaled** so no two share a silhouette, **foliage that sways in the wind**, and
> a whole field that **thins into the distance** instead of ending at a wall. The universe is now a **hierarchy of galaxies**: you start inside the Milky Way, and if
...
| ![A distant spiral galaxy reticled in intergalactic space](Screenshots/011.jpg)<br>**Galaxy reticle** — out in intergalactic space the centre crosshair locks onto the galaxy it's aimed at, reading out its catalog name, morphology, star count and distance (M 33 · Spiral · 419.7 B stars · 525 kly). | ![Inside a galaxy with the bright bulge, resolved star cloud and core reticle](Screenshots/012.jpg)<br>**Inside a galaxy** — the volumetric star cloud and glowing bulge resolve around you on crossing into a galaxy, the magenta CORE reticle marking the supermassive black hole 21.7 kly ahead. |
| ![A dwarf galaxy resolved into stars amid coloured nebulae](Screenshots/013.jpg)<br>**Dwarf among nebulae** — a small irregular galaxy resolved into a loose knot of stars, strung through with the green-and-pink nebula field of the surrounding halo. | ![A faraway galaxy as a point sprite with an aimed reticle](Screenshots/014.jpg)<br>**Point-sprite galaxy** — 16 Mly out, UGC 12612 is still a single direction-only point sprite, yet the aimed reticle already reports it as a Spiral of 452.8 B stars. |
| ![The same spiral seen edge-on as an impostor disk](Screenshots/015.jpg)<br>**Impostor disk** — closing to 180 kly, the same UGC 12612 cross-fades into a procedurally shaded edge-on disk, bulge and dust lane catching the light. | ![A globular cluster reticle with an off-screen core arrow](Screenshots/016.jpg)<br>**Globular + core arrow** — a halo globular cluster marked by its gold GC reticle while the magenta CORE arrow clamps to the screen edge, pointing back to the galactic centre 23 kly away. |
| ![A globular cluster shown as a single fuzzy sprite](Screenshots/017.jpg)<br>**Fuzzy sprite** — from thousands of ly out a globular cluster reads as one soft sprite, the core arrow still tracking the galaxy's heart off-frame. | ![A globular cluster resolved into individual stars](Screenshots/018.jpg)<br>**Cluster resolves** — closing in at 14.5 c, the sprite cross-fades into the cluster's real stars: a dense golden swarm that holds its brightness right up until you're among them. |
| ![Among a cluster's stars with named star reticles](Screenshots/019.jpg)<br>**Named stars** — inside the cluster every nearby star carries a deterministic name (Tycho IX, Onyx Lambda, Phoenix XIV) beside its class and distance, the nearest one bracketed in green. | |
...
| `Game.Universe` | procedural generation. **Universe of galaxies** — `Galaxy` + `GalaxyCatalog`/`GalaxyCatalogPager` (a sparse lattice of galaxies, the `INearestGalaxy` source), `GalaxyField` (cosmic density), `GalaxyModel` (one galaxy's stellar density: disk/arms/bulge), `GalaxyId`, `GalaxyCloud` (a galaxy's volumetric point set) and `GalaxyGlow` (its nebula-style body-glow puffs). **Tiled star lattice** confined to galaxy interiors — `StarCatalog` (one indexed block; injects globular-cluster stars), `StarCatalogPager` (loads/evicts blocks around the camera, the `INearestStar` source) and `StarId` (packs the global, invertible catalog id). **Globular clusters** — `GlobularCluster`/`GlobularClusters` (halo clusters + their shared per-star generator). Plus `BackdropStars`, `NebulaField` (per-galaxy), `SystemGenerator` (planets + moons), `Noise` (fBm + ridged), `PlanetTerrain`, atmospheres, the `Rover` surface-physics sim, the live knobs (`TerrainTuning`/`BiomeTuning` globals + `PlanetTuning` per-type overrides), and the legacy `StarField` cell-streamer |
...
**Galaxies & clusters.** Fly out of the Milky Way and the other galaxies appear — a **fuzzy point** far
off (intergalactic space shows a deep field of them) that **swells into a bright smudge** as you approach,
then resolves into a tilted **spiral or elliptical disk** (glowing bulge + dark dust lanes), then a
**volumetric body glow** — billowing nebula-style gas you fly *through*, brightest in the warm core — over
a **3-D star cloud**, and finally the galaxy's real streamed stars as you cross inside. As you enter a
galaxy the distant faint galaxies **fade out** (only the near, bright ones stay), and they fade back in as
you leave. Each galaxy has its own central black hole — flagged inside the galaxy by a **`CORE` reticle**
(with an off-screen arrow) so you can fly to the supermassive black hole — plus its own nebulae and a halo
of **globular clusters**: from thousands of ly away each reads as a fuzzy star (tagged on the HUD with a
`GC` bracket reticle + distance), and as you close in it resolves into a dense ball of **real stars you can
visit** — every one spawns its own solar system, just like the disk stars, with the core spaced so you can
actually fly between them. The Navigation panel lists the nearest galaxies with a **Go** button (a few
radii out, framed) so the millions-of-ly voids are actually crossable; intergalactic space itself is empty
(stars exist only inside galaxies). The galaxy look (point/impostor/cloud/glow brightness and sizes, cloud
point size, and the globular sprite/star brightness) is live-tunable in the Tuning panel and saved to
`tuning.json`.
...
points to it back in fly view). Press `U` for the **universe map** — the tier above: a pannable, zoomable
top-down chart of the **galaxies** in the cosmic lattice, each a tinted disk sized to its true radius (the
galaxy you're inside is ringed); hover for its name, click to read its type/radius/star-count/distance and
*Jump here*. (The galaxy and universe maps currently show the resident bubble; streaming new
stars/galaxies in as you roam past it is a follow-up.)
...
impostor and cloud brightness, cloud point size — fewer/larger for performance, and the volumetric
body-glow on/off + brightness), the **globular clusters** (sprite and resolved-star brightness, star
size), the **galaxy backdrop** (an on/off
...
- [x] **Multi-spawner surface-object scatter** — any number of layers, each with its own mesh/density/size/orientation, gated per world by environment traits + a spawn-chance roll (see the next bullet for the real geometry)
- [x] **Procedural trees & rocks** — the scatter layers now render as lit low-poly geometry instead of debug solids: lump-displaced boulders that sit half-embedded, trunk+canopy broadleaf trees and tiered conifers, height-ramped vertex colours with base-contact AO + biome tint, per-instance non-uniform scale (no two alike), wind-swayed foliage, and a distance LOD that thins the field toward the horizon
...
- [x] **Sun disk + lens flare** — a screen-space lens flare for the active star: a radiating core (disc + six-point starburst + anamorphic streak) plus a chain of chromatic ghost discs and halo rings strung along the sun→screen-centre axis, drawn additively over the final image. Faded by an analytic ray-vs-sphere **occlusion** test (planets/moons crossing the sightline eclipse it, so it sets behind a limb instead of shining through), by off-screen drift, and by a behind-camera gate; fully procedural, HUD strength knob
- [x] **Shoreline foam** — an animated, broken band of white foam pooling in the shallows along ocean coastlines (keyed to water depth over the sea floor) plus a light dusting on wave crests; procedural surf noise, HUD strength knob
- [~] **SSAO** — screen-space ambient occlusion reconstructed from the scene depth buffer alone (no G-buffer): view-space position + normal recovered per pixel, hemisphere-sampled, half-res + blurred, multiplied into the scene at composite time. Built with HUD radius/bias/contrast/strength knobs but shipped **disabled** — the metres-to-thousands-of-km depth range quantises the buffer enough that the depth-derived normals band and noise on distant terrain; usable planetary SSAO needs a linear-depth prepass + distance gating
...
**Next up**
- [x] **Surface-object meshes** — real procedural boulders and trunk+canopy trees (broadleaf + tiered conifer) replacing the placeholder solids, lit + biome-tinted, per-instance shaped, wind-swayed, distance-thinned
- [ ] **Grass & detail props** — low grass tufts, flowers and small clutter as further scatter layers (the tree/rock layers are now real geometry)
...
- [x] Multi-spawner surface-object scatter (instanced on the drawn surface; per-layer mesh/density/size/orientation + env-trait & spawn-chance gating) — now rendered as **real procedural trees & boulders** (lit, biome-tinted, per-instance shaped, wind-swayed, distance-thinned); grass tufts pending
- [ ] Rivers & lakes (flow-routed hydrology carving the terrain)
...
- [x] Shoreline foam (animated broken foam band pooling in the shallows + wave-crest dusting, in the water shader)
- [~] SSAO (built — screen-space AO reconstructed from scene depth, half-res + blurred, HUD knobs — but shipped **disabled**: the huge planetary near/far range quantises the depth buffer, so the depth-reconstructed normals band/noise on distant terrain; needs a linear-depth prepass + distance gating to be usable)
- [ ] Terrain self-shadowing, water reflections / SSR
- [x] Sun disk + lens flare (screen-space: radiating core + chromatic ghost/halo chain, analytic occlusion fade, HUD strength knob)
- [ ] Spectral star colours by temperature
