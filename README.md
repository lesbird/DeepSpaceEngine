...
**Assets are optional.** The repo ships none, so by default the UI click is generated by a small
procedural synth and **no music plays** (the synth ambient pad is a deep drone that reads as a constant
hum, so it's opt-in via the Audio panel — never automatic). Drop `blip.wav` and/or `music.wav` into
`Game.App/Assets/Audio/` and they're picked up automatically: the WAV is loaded (uncompressed PCM,
8/16-bit mono/stereo) and music auto-plays on launch, looped.
