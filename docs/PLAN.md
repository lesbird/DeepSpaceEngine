...
We're building a 3D space-exploration game engine. The defining requirement is **true-scale
distances**: billions of stars, light-years apart, each with a deterministic solar system derived
from the star's index, with seamless transitions from interstellar flight → solar system → planet
surface → driving a rover.
...
- Language/runtime: **C# on .NET** — currently targets **net7.0** (the plan called for net8 LTS;
  it's a one-line `<TargetFramework>` change in `Directory.Build.props` once that SDK is installed)
- GL bindings: **Silk.NET 2.23.0** (GL + GLFW windowing + input + Silk.NET.Maths)
- Target: **cross-platform, OpenGL 4.1 baseline** (runs on the user's Mac; no compute shaders).
  Procedural generation runs CPU-side with `Parallel.For` + a background worker pool **and**, for
  terrain, on the GPU via fragment-shader passes baked into texture tiles (no compute required)

---

## Current state — 2026-06-20

**Milestones M0–M4 are complete; M5 (fidelity) is well underway.** The engine builds and runs;
the `Engine.Core.Tests` xUnit suite (64 facts/theories: coordinate precision, hashing/determinism,
star field, solar systems, terrain, rover, rings, asteroids, backdrop, speed policy) is green.

What works today:
- **Hierarchical coordinates + floating origin** (`UniversePosition`) — sub-mm precision, no jitter
  at any distance. Unit-tested.
- **Unbounded tiled star catalog** — a distance-paged lattice of star blocks (`StarCatalog` +
  `StarCatalogPager`); billions addressable, a few million resident, each with a stable invertible
  catalog id you can **search and jump** to (loads any block on demand). `StarField` is the legacy
  cell-streamer; `BackdropStars` + `GalaxyBackdrop` draw the far-field dome + Milky-Way band.
- **Deterministic solar systems** — spawn/despawn at 0.5 ly with hysteresis (`SolarSystemManager`):
  emissive sun + glow, lit Keplerian planets/moons, orbit rings, banded gas/ice giants with rings,
  asteroid belts (`AsteroidFieldManager`).
- **Landable planets** — cube-sphere quadtree-LOD terrain, **GPU-generated by default**
  (`TerrainTileGenerator`/`TerrainTileCache` bake height/normal/albedo tiles, displaced via vertex
  texture fetch), CPU worker-pool bake as a fallback. Smooth fractional-octave LOD + geomorph,
  ridged-multifractal mountains, craters + maria, oceans, erosion, micro-relief/strata, orbital
  macro-relief matching the surface. Slope-aware biome colouring (Whittaker matrix), lava worlds,
  city lights. Composition-driven **volumetric atmospheres** (Rayleigh + Mie) + a body scanner.
- **Drivable rover** (`R`) — per-planet gravity, collision mesh of the drawn triangles, damped
  per-wheel suspension, chase camera.
- **Proximity speed limiter** (`SpeedPolicy`) — unlimited in open space, self-converging slowdown
  near bodies, anti-tunnelling.
- **Multi-spawner surface scatter** — instanced objects on the drawn terrain, per-layer
  mesh/density/size/orientation, gated by environment traits + a spawn-chance roll.
- **System & galaxy maps** — `M` top-down system schematic (click-to-travel), `N` 2D galaxy chart
  (click-to-jump) plus a navigable 3-D neighbourhood view (orbit cam, 10–50 ly, nearest-neighbour
  route, click-to-target).
- **HUD + live tuning panel** — atmosphere/relief/biome/scatter knobs saved to `tuning.json`; `H`
  toggles all UI.

See the [README](../README.md) for the full feature writeup, screenshots and controls. The detailed
design rationale below still describes the shipped architecture.
...
- **M0 — Foundation.** ✅ Silk.NET window + GL 4.1 context, main loop, clear/present, ImGui overlay,
  ...
  space with a debug reference grid. *Exit met: fly around an empty scene smoothly.*
- **M1 — Star field & true scale.** ✅ Hierarchical coords live; procedural galaxy placement;
  instanced point-sprite star rendering with blackbody color; sector streaming + frustum cull;
  log-scale mouse-wheel speed; HUD (position, speed, nearest star, FPS); on-screen reticles +
  nearest-star arrow. *Exit met: fly light-years with zero jitter; billions addressable, thousands
  rendered.*
- **M2 — Solar systems.** ✅ `StarId` → system generator; spawn/despawn at 0.5 ly with hysteresis;
  emissive sun + lit planet/moon spheres on Keplerian orbits + orbit rings; approach/transition into
  a system. *Exit met: fly to a star, its system appears; leave, it despawns.*
- **M3 — Planets & terrain.** ✅ Cube-sphere quadtree-LOD terrain from planet seeds (GPU-baked tiles
  by default, CPU fallback); ridged mountains + domain warp + per-LOD detail; smooth fractional-octave
  LOD + geomorph; ocean worlds; volumetric atmospheres; live tuning HUD with `tuning.json`. Descend
  and land with floating-origin precision. *Exit met: land on a planet surface, no jitter.*
- **M4 — Rover.** ✅ Per-planet gravity, terrain-follow + slope tilt, ledge falls/landings,
  collision mesh of the drawn triangles, damped per-wheel suspension, throttle/steer under a chase
  camera, free-fly ↔ rover state switching. *Exit met: land, deploy a rover, drive across terrain.*
- **M5 — Fidelity (in progress).** Shipped so far: distant-galaxy backdrop; smooth terrain LOD;
  planetary rings; threaded terrain generation; proximity speed limiter; tiled star catalog;
  composition-driven atmospheres + scanner; **GPU terrain path (now default)** with full CPU parity
  (craters/maria, oceans, erosion, micro-relief/strata, orbital macro-relief); biome expansion; lava
  worlds; city lights; banded gas giants; multi-spawner surface scatter; system & galaxy maps (2D + a
  3-D neighbourhood view). Held back: **eclipses** (built but disabled — too abrupt) and **planet
  rotation / day-night** (prototyped, dropped — needs true geometric spin). *Next:* real
  surface-object meshes (vs. placeholders), surface-aware LOD, rivers & lakes, eclipses take two.
- **Backlog (beyond M5).** Binary systems, comets, aurorae, HDR/tonemapping, terrain self-shadowing,
  player ship + lander, first-person on-foot, location bookmarks, audio, **net8 LTS migration**.
  See the README's *Ideas & backlog* for the full list.
